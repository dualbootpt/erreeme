...
class LangMenuModel {
	...
	const SQL_TABLE_NAME='LANG_MENU';
	const SQL_INSERT='INSERT INTO `LANG_MENU` (`autoid`,`lang`,`menu`,`option`,`text`) VALUES (?,?,?,?,?)';
	const SQL_UPDATE='UPDATE `LANG_MENU` SET `autoid`=?,`lang`=?,`menu`=?,`option`=?,`text`=? WHERE `autoid`=?';
	const SQL_SELECT_PK='SELECT * FROM `LANG_MENU` WHERE `autoid`=?';
	const SQL_DELETE_PK='DELETE FROM `LANG_MENU` WHERE `autoid`=?';
	const FIELD_AUTOID=0;
	const FIELD_LANG=1;
	const FIELD_MENU=2;
	const FIELD_OPTION=3;
	const FIELD_TEXT=4;
	...
	public function &setLang($lang) {
		$this->notifyChanged(self::FIELD_LANG);
		$this->lang=$lang;
		return $this;
	}
	...
	public function &setMenu($menu) {
		$this->notifyChanged(self::FIELD_MENU);
		$this->menu=$menu;
		return $this;
	}
	...
	public function &setOption($option) {
		$this->notifyChanged(self::FIELD_OPTION);
		$this->option=$option;
		return $this;
	}
	...
	public function &setText($text) {
		$this->notifyChanged(self::FIELD_TEXT);
		$this->text=$text;
		return $this;
	}
	...
	public function toArray() {
		return array(
			...
			self::FIELD_MENU=>$this->getMenu(),
			self::FIELD_OPTION=>$this->getOption(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of LangMenuModel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param LangMenuModel $example an example instance
	 * @return LangMenuModel[]
	 */
	public static function findByExample(PDO $db,LangMenuModel $example, $and=true) {
	 ...
	 *
	 * Will return matched rows as an array of LangMenuModel instances.
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $filter
	 * @param boolean $and
	...
	/**
	 * Execute select query and return matched rows as an array of LangMenuModel instances.
	 *
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getMenu());
		$stmt->bindValue(4,$this->getOption());
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(6,$this->getAutoid());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		$doc=new DOMDocument();
		$root=$doc->createElement('LangMenuModel');
	...
	/**
	 * get single LangMenuModel instance from a DOMElement
	 *
	 * @param DOMElement $node
	 * @return LangMenuModel
	 */
	public static function fromDOMElement(DOMElement $node) {
		if ('LangMenuModel'!=$node->nodeName) {
			return new InvalidArgumentException('expected: LangMenuModel, got: ' . $node->nodeName, 0);
		}
		$result=array();
	...
	/**
	 * get all instances of LangMenuModel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('LangMenuModel') as $node) {
